...
const ExpenseTracker = () => {
  ...
  const getMonthOptions = () => {
    ...
    for (let y = startDate.getFullYear(); y <= currentDate.getFullYear(); y++) {
      ...
      let endM = y === currentDate.getFullYear() ? currentDate.getMonth() + 1 : 12;
  ...
  useEffect(() => {
    ...
    if (sessionBudget <= 0) {
      toast.info("Будь ласка, вкажіть ваш бюджет");
      setTimeout(() => {
        setBudgetModal(true);
      }, 500);
    }
  ...
  return (
    <div className="container relative mx-auto pb-20 p-4">
      ...
      <div className="flex justify-between text-blue_c items-start pt-4">
        <div>
          <h1 className="font-bold text-2xl">Транзакції</h1>
          {earliestDate && (
      ...
      {expenses.length > 0 ? (
        ...
        <h3 className="text-center w-max mx-auto pt-5">Немає транзакцій</h3>
      )}
    </div>
  );
};
